...
type ApplicationTab = 'all' | 'APPLIED' | 'REVIEWING' | 'SHORTLISTED' | 'INTERVIEWED' | 'HIRED' | 'REJECTED';
...
const Icons = {
  ...
  Verified: () => (
    <svg viewBox="0 0 24 24" className="h-4 w-4 text-[#079F67]" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>
  ),
};
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold border uppercase tracking-wider ${colorClasses}`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex gap-4 min-w-0">
          ...
          <div className="space-y-1 min-w-0">
            <h2 className="text-lg font-bold text-[#0F132E] truncate leading-none capitalize group-hover:text-[#079F67] transition-colors">{title}</h2>
            <div className="flex items-center gap-1 text-sm font-semibold text-slate-500">
              {job?.employerSnapshot?.displayName || 'Employer'} <Icons.Verified />
            </div>
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-1 text-slate-400 font-medium text-xs">
              <div className="flex items-center gap-1.5 capitalize"><Icons.Location /> {job?.location?.city || 'Remote'}</div>
              <div className="flex items-center gap-1.5 text-[#079F67]"><span className="w-1.5 h-1.5 rounded-full bg-current" /> {job?.jobType?.toLowerCase().replace('_', '-') || 'Full-time'}</div>
              <div className="flex items-center gap-1.5"><Icons.Calendar /> Applied {new Date(app.createdAt).toLocaleDateString()}</div>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-3 w-full md:w-auto self-end md:self-center border-t md:border-none pt-4 md:pt-0">
          <StatusBadge status={app.status} />
          <Link href={`/jobs/${actualJobId}`} className="flex-1 md:flex-none px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#079F67] to-[#0F132E] text-white text-sm font-bold hover:opacity-90 transition-all text-center shadow-lg active:scale-95">
            View Details
          </Link>
        </div>
      </div>
...
export function JobseekerApplicationsPage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-white flex">
      <div className="hidden md:block sticky top-0 h-screen shrink-0"><JobseekerSidebar /></div>
