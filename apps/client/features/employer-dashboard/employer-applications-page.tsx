...
const Icons = {
  Back: () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M19 12H5M12 19l-7-7 7-7" /></svg>,
  Chevron: () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="m6 9 6 6 6-6" /></svg>,
  Briefcase: () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5"><rect width="20" height="14" x="2" y="7" rx="2" ry="2" /><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16" /></svg>,
  User: () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>,
};
...
export function EmployerApplicationsPage() {
  ...
  const filteredList = useMemo(() => {
    return applications.filter((app) => {
      ...
      const fullName = `${app.applicantSnapshot?.firstName} ${app.applicantSnapshot?.lastName}`.toLowerCase();
      return matchesStatus && fullName.includes(searchTerm.toLowerCase());
    });
  }, [applications, statusFilter, searchTerm]);

  // Derived Title for the Page
  const pageTitle = jobId && applications.length > 0 
        ...
        <section className="flex-1 flex flex-col min-w-0">
          <div className="p-6 md:p-10 max-w-7xl mx-auto w-full">
            ...
            <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
              <div className="min-w-0">
                <div className="flex items-center gap-3 mb-2">
                   {jobId && (
                     <Link href="/dashboard/employer/my-jobs" className="p-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors shadow-sm">
                        <Icons.Back />
                     </Link>
                   )}
                ...
                <p className="text-slate-500 font-medium ml-1">
                  {jobId ? 'Showing candidates for this specific listing' : 'Review all incoming applications across your postings.'}
                </p>
              </div>
            ...
            <div className="flex p-1 bg-slate-200/50 rounded-xl w-fit mb-8 overflow-x-auto max-w-full">
              {['ALL', STATUS.APPLIED, STATUS.REVIEWING, STATUS.SHORTLISTED, STATUS.REJECTED, STATUS.ACCEPTED].map((f) => (
                <button
                  key={f}
                  onClick={() => setStatusFilter(f)}
                  className={`px-6 py-2.5 rounded-lg text-xs font-black uppercase transition-all whitespace-nowrap ${
                    statusFilter === f ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-800"
                  }`}
                >
            ...
            <div className="space-y-4">
              {loading ? (
                    ...
                    <div className="flex gap-5 flex-1 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-sm font-semibold text-teal-600 mt-1">
                          <Icons.Briefcase />
                          <span className="truncate">Applied for: {app.jobId?.title || 'Unknown Position'}</span>
                        </div>

                        <p className="text-sm font-medium text-slate-400 mt-1">
                          Current: {app.applicantSnapshot?.currentPosition || 'Applicant'}
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="flex flex-col md:flex-row items-center gap-3">
                        ...
                        <div className="relative">
                          ...
                          >
                            ...
                            <option value={STATUS.ACCEPTED}>Hire (Accept)</option>
                ...
                <div className="py-24 text-center bg-white rounded-3xl border-2 border-dashed border-slate-200">
                   <p className="text-slate-400 font-bold uppercase tracking-widest italic">No matching applications found</p>
                </div>
              )}
            </div>
          </div>
