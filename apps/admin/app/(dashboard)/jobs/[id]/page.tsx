...
export default function JobDetailPage() {
  ...
  const job = {
    ...
    description: "We are looking for a Senior React Developer to join our core engineering team. You will be responsible for building high-quality, performant web applications and mentoring junior developers.",
    requirements: [
      ...
      <header className="px-10 py-6 border-b border-slate-100 bg-white sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black text-slate-900 tracking-tight">Post Verification</h1>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">Job ID: {id}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-1 space-y-8">
            <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
              ...
              <h2 className="text-2xl font-black text-slate-900 tracking-tight leading-tight mb-2">{job.title}</h2>
              <p className="text-emerald-600 font-black text-sm uppercase tracking-wider mb-6">{job.employer}</p>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 text-slate-500">
                  ...
                  <span className="text-xs font-bold">Posted: {job.postedDate}</span>
                </div>
              </div>
              ...
              <div className="mt-8 pt-8 border-t border-slate-50">
                <div className={cn(
                  ...
                  job.status === "Pending" ? "bg-amber-50 text-amber-600" : "bg-emerald-50 text-emerald-600"
                )}>
            ...
            <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Candidate Requirements</h3>
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-slate-700">{job.education}</p>
                  </div>
                </div>
              </div>
            </section>
          ...
          <div className="lg:col-span-2 space-y-8">
            <section className="bg-white rounded-[2rem] p-10 shadow-sm border border-slate-100">
               <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-50">
                  ...
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Job Description</h3>
               </div>
               <p className="text-slate-600 font-bold leading-relaxed">{job.description}</p>
            </section>

            <section className="bg-white rounded-[2rem] p-10 shadow-sm border border-slate-100">
               <div className="flex items-center gap-3 mb-8 pb-4 border-b border-slate-50">
                  <CheckCircle2 className="w-5 h-5 text-emerald-500" />
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Key Requirements</h3>
               </div>
               <ul className="space-y-4">
                  {job.requirements.map((req, idx) => (
                    <li key={idx} className="flex items-start gap-4">
                      <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-emerald-500 flex-shrink-0" />
                      <span className="text-sm font-bold text-slate-700 leading-tight">{req}</span>
                    </li>
                  ))}
               </ul>
            </section>
            ...
            {job.status === "Pending" && (
              <section className="bg-[#0F172A] rounded-[2rem] p-10 shadow-2xl text-white">
                <h3 className="text-sm font-black uppercase tracking-[0.2em] mb-4 text-white/60">Admin Action Required</h3>
                <p className="text-white/80 font-medium mb-8">
                  Review the job posting details carefully before approval. Once approved, the job will be visible to all jobseekers on the platform.
                </p>
                <div className="flex items-center gap-4">
                   <button 
                     onClick={handleApprove}
