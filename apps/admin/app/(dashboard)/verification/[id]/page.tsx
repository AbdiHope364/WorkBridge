      ...
      <header className="px-10 py-6 border-b border-slate-100 bg-white sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black text-slate-900 tracking-tight">Review Application</h1>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">Request ID: {id}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-1 space-y-8">
            <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
               <div className="flex items-center gap-4 mb-6">
                  <div className={cn(
                    ...
                    request.userType === "Jobseeker" ? "bg-blue-50 text-blue-600 border-blue-100" : "bg-emerald-50 text-emerald-600 border-emerald-100"
                  )}>
                    {request.applicantName.charAt(0)}
                  </div>
                  <div>
                    <h2 className="text-xl font-black text-slate-900 tracking-tight">{request.applicantName}</h2>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{request.userType}</p>
                  </div>
               </div>
               ...
               <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                     ...
                     <div className="text-right">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Submitted</p>
                        <p className="text-xs font-black text-slate-800">{request.submittedDate}</p>
                     </div>
                  </div>
            ...
            <section className="bg-[#0F172A] rounded-[2rem] p-10 shadow-2xl text-white">
               <h3 className="text-sm font-black uppercase tracking-[0.2em] mb-4 text-white/50">Verification Guidelines</h3>
          ...
          <div className="lg:col-span-2 space-y-8">
            <section className="bg-white rounded-[2rem] p-10 shadow-sm border border-slate-100">
               <div className="flex items-center gap-3 mb-8 pb-4 border-b border-slate-50">
                  ...
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Submitted Documents</h3>
               </div>
                  ...
                  {request.documents.map((doc, idx) => (
                    <div key={idx} className="group relative">
                       <div className="aspect-[4/3] bg-slate-50 rounded-[1.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center p-6 transition-all group-hover:border-emerald-300 group-hover:bg-emerald-50/30">
                          ...
                          <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">{doc.type} • {doc.size}</p>
                       </div>
            ...
            <section className="bg-white rounded-[2rem] p-10 shadow-sm border border-slate-100">
                <div className="flex items-center justify-between gap-6">
                   <div className="flex-1">
                      <h4 className="text-sm font-black text-slate-900 mb-2">Are the documents valid?</h4>
                      <p className="text-xs font-bold text-slate-400">By verifying this user, you confirm that their identity or business documents are legitimate and up to date.</p>
                   </div>
                   <button 
                     onClick={handleVerify}
